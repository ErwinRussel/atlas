...
//     Setup sigusr criu_signal_handler()
    ...
    // Setup criu stuff
    int fd = open("/home/erwinrussel/Documents/atlas/dev/checkpoints", O_DIRECTORY);
    criu_init_opts();
...
// -- XCreateSimpleWindow
Window xcreatesimplewindow_uh(args_XCreateSimpleWindow argp)
{
    // Memcopy in Buffer
    int arg_size = sizeof(args_XCreateSimpleWindow);
    memcpy(ShmPTR->buffer, &argp, arg_size);
...
// -- XSelectInput
int xselectinput_uh(args_XSelectInput argp)
{
    // Memcopy in Buffer
    int arg_size = sizeof(args_XSelectInput);
    memcpy(ShmPTR->buffer, &argp, arg_size);
...
// -- XDefaultRootWindow
Window xdefaultrootwindow_uh(args_XDefaultRootWindow argp)
{
    // Memcopy in Buffer
    int arg_size = sizeof(args_XDefaultRootWindow);
    memcpy(ShmPTR->buffer, &argp, arg_size);
